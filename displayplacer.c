    ...
    for (int i = 0; i < argc - 1; i++)
    {
        ...
        while (propSetToken)
        {
            ...
            case 'q': // supress error when screen by ID isnt found
                ...
                break;
            default:
                fprintf(stderr, "Argument parsing error: %c\n", propToken[0]);
                exit(1);
            }
...
void printVersion()
{
    printf(
        ...
        "\n"
        "Developer: Jake Hilborn`n"
        "modifications by mnbf9rca to ignore missing screens\n"
        "GitHub: https://github.com/jakehilborn/displayplacer -> https://github.com/mnbf9rca/displayplacer\n"
...
void printCurrentProfile()
{
    ...
    for (int i = 0; i < screenCount; i++)
    {
        if (CGDisplayIsInMirrorSet(screenConfigs[i].id) && CGDisplayMirrorsDisplay(screenConfigs[i].id) != 0)
        { // this screen is a secondary screen in a mirroring set
    ...
    for (int i = 0; i < screenCount; i++)
    {
        ...
        if (curScreen.id == -1)
        { // earlier we set this to -1 since it will be represented as a mirror on output
            continue;
        }
...
bool configureResolution(CGDisplayConfigRef configRef, CGDirectDisplayID screenId, char *screenUUID, int width, int height, int hz, int depth, bool scaled, int modeNum)
{
    ...
    if (modeNum != -1)
    { // user specified modeNum instead of height/width/hz
    ...
    for (int i = 0; i < modeCount; i++)
    {
        ...
        if (curMode.derived.width != width)
            continue;
        if (curMode.derived.height != height)
            continue;
        if (hz && curMode.derived.freq != hz)
            continue;
        if (depth && curMode.derived.depth != depth)
            continue;
        if (scaled && curMode.derived.density != 2.0)
            continue;
        if (!scaled && curMode.derived.density == 2.0)
            continue; // fix for scaling
